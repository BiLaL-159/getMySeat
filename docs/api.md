...
|---|---|---|
| `POST /api/v1/shows/{id}/holds` | Customer | Holds `{ "seats": [seatId…], "generalAdmission": [{ "sectionId", "quantity" }…] }`, 1 to 10 tickets in all, and returns `201` with the Hold. Needs an `Idempotency-Key` header of 1 to 255 characters (`400` otherwise); see below. Each Seat and Section must be part of the Show's Venue, appear once, and be asked for through the right kind of item (`400` otherwise). If anything is gone, nothing is held and the `409` has the type `urn:getmyseat:problem:inventory-unavailable` with `unavailableSeats` (Seat ids) and `unavailableSections` (`{ sectionId, available }`). A deadlock or serialization failure gets the same `409`, so just retry. A draft or unknown Show is `404`; a Show that has started is `409`. |
...
Every Hold creation carries an `Idempotency-Key`, so a client retrying after a network failure never holds twice, and never releases the Hold its first attempt made. Keys are scoped per Customer. A retry with the same key and the same request (the Show id and the body, whatever order its items are in) returns `201` with the original Hold as it is now, perhaps `RELEASED` or `EXPIRED`, and claims nothing. The same key with a different request is a `409` with the type `urn:getmyseat:problem:idempotency-key-reused`. The key is written in the Hold's transaction, before the previous Hold for the Show is released, so a request that fails (`400`, `404` or `409`) leaves the key unused and its retry tries again, and of several requests racing with one key, exactly one makes a Hold and the rest wait for it and return it. The cleanup job forgets keys after 24 hours.
